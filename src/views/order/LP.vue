<template>
  <div class="lp-order">
    <a-card :title="t('title.lpOrderQuery')" class="filter-card">
      <a-form layout="vertical" :model="queryParams" class="query-form">
        <div class="form-row">
          ...
          <a-form-item :label="t('field.orderType')" class="form-item-md">
            <lp-order-type-select
      ...
      >
        <template #bodyCell="{ column, text, record }">
          ...
          <template v-else-if="column.key === 'orderType'">
            <a-tag :color="record.orderType === 'add' ? 'green' : 'orange'">
              {{ t(`orderType.${record.orderType}`) }}
            </a-tag>
          </template>

          <template v-else-if="column.key === 'transactionAmount1'">
            <div class="amount-info">
              <div class="info-item">
                <span class="label">{{ t('field.orderTransactionAmount1') }}:</span>
                <span>{{ record.orderTransactionAmount1 }}</span>
              </div>
              <div class="info-item">
                <span class="label">{{ t('field.actualTransactionAmount1') }}:</span>
                <span>{{ record.actualTransactionAmount1 }}</span>
              </div>
            </div>
          </template>

          <template v-else-if="column.key === 'transactionAmount2'">
            <div class="amount-info">
              <div class="info-item">
                <span class="label">{{ t('field.orderTransactionAmount2') }}:</span>
                <span>{{ record.orderTransactionAmount2 }}</span>
              </div>
              <div class="info-item">
                <span class="label">{{ t('field.actualTransactionAmount2') }}:</span>
                <span>{{ record.actualTransactionAmount2 }}</span>
              </div>
            </div>
          </template>
...
import LpStatusSelect from '@/components/selectors/LpStatusSelect.vue'
import LpOrderTypeSelect from '@/components/selectors/LpOrderTypeSelect.vue'
import '@/styles/common.css'
import zhLocale from '@/locales/order/LP/zh.json'
import enLocale from '@/locales/order/LP/en.json'
import mockData from '@/mock/order/LP/lp.mock.json'
...
interface LpRecord {
  ...
  transferId: string
  txHash: string
  orderType: 'add' | 'remove'
  chainType: string
  transactionCurrency1: string
  orderTransactionAmount1: string
  actualTransactionAmount1: string
  transactionCurrency2: string
  orderTransactionAmount2: string
  actualTransactionAmount2: string
  orderStatus: 'pending' | 'processing' | 'lpSuccess' | 'lpFailed' | 'manualConfirm'
...
const columns = computed(() => [
  ...
  {
    title: t('field.transactionCurrency1'),
    dataIndex: 'transactionCurrency1',
    key: 'transactionCurrency1',
    width: 120,
  },
  {
    title: t('field.transactionAmount1'),
    dataIndex: 'transactionAmount1',
    key: 'transactionAmount1',
    width: 150,
  },
  {
    title: t('field.transactionCurrency2'),
    dataIndex: 'transactionCurrency2',
    key: 'transactionCurrency2',
    width: 120,
  },
  {
    title: t('field.transactionAmount2'),
    dataIndex: 'transactionAmount2',
    key: 'transactionAmount2',
    width: 150,
  },
  {
...
const getOrderStatusText = (status: LpRecord['orderStatus']): string => {
  return t(`status.${status}`)
}

const getNotifyStatusText = (status: LpRecord['notifyStatus']): string => {
  return t(`status.${status}`)
}
...
const handleRetryNotify = (record: LpRecord) => {
  ...
  notifyContent.value = {
    ...
    transactionAmount1: record.actualTransactionAmount1,
    transactionAmount2: record.actualTransactionAmount2,
...
const currentRecord = ref<LpRecord | null>(null)
...
const handleNotifyHistory = (record: LpRecord) => {
  currentRecord.value = record
  notifyHistoryVisible.value = true
  notifyHistoryData.value = mockData.notifyHistoryData as NotifyHistoryRecord[]
}

const tableData = ref<LpRecord[]>(mockData.tableData as unknown as LpRecord[])
...
const handleDownload = () => {
  ...
  const rows = tableData.value.map(row => {
    return columnsArray.map(col => {
      const value = row[col.dataIndex as keyof LpRecord] || ''
      return `"${value}"`
    }).join(',')
  })
  ...
  link.setAttribute('download', `lp_orders_${new Date().toISOString().split('T')[0]}.csv`)
