...
The system SHALL provide alert threshold percentage configuration functionality for collection wallet and withdrawal wallet storage limits in merchant management currency configuration interface. 系統必須在商戶管理的幣種配置介面中,為歸集錢包和出款錢包的儲存上限提供告警水位比例設定功能。
